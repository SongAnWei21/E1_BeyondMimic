...
class MySceneCfg(InteractiveSceneCfg):
    #运动跟踪任务的场景配置,包含地形、机器人、照明和传感器的完整场景设置。
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        def __post_init__(self):
            ...
            self.history_length = 10           # 10帧历史数据
            self.enable_corruption = True      # 启用观察破坏（噪声）
            self.concatenate_terms = True      # 将所有观察项连接为一个向量，False则保持观测数据的字典结构
            #self.flatten_history_dim = True   # IsaacLab 很多旧版本默认就是 True, 扁平化
    ...
    class PrivilegedCfg(ObsGroup):
        ...
        projected_gravity = ObsTerm(func=mdp.projected_gravity)  # 重力投影
        ...
        def __post_init__(self):
            ...
            self.enable_corruption = False # 是否观测值加入噪声
            self.concatenate_terms = True # 是否合并观测项，将所有观察项连接为一个向量，False则保持观测数据的字典结构
...
class EventCfg:
    ...
    randomize_mass = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=".*"),
            "mass_distribution_params": (0.85, 1.15),  # 全身各个连杆质量上下浮动 15%
            "operation": "scale",
        },
    )
    ...
    randomize_joint_friction = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*"]),
            "friction_distribution_params": (0.0, 0.05), # 增加 0 到 0.05 N.m 的随机干摩擦力 (根据电机量级微调)
            "operation": "add",
        },
    )
...
class RewardsCfg:
    ...
    #     weight=-1e-4,  # 给一个轻微的惩罚，限制过大的关节速度
    # )

    # 关节加速度惩罚 
    joint_acc_l2 = RewTerm(
        func=mdp.joint_acc_l2,
        weight=-1e-5,  # 限制关节加速度，使动作更具韧性
    )
    ...
    # 力矩/能量惩罚,目前动作大概率是全程满扭矩输出，加上这个可以显著降低真机发热和生硬感
    dof_torques_l2 = RewTerm(
        func=mdp.joint_torques_l2,
        weight=-1e-4, # 需要根据你电机的实际扭矩数量级微调
    )
...
class TerminationsCfg:
    ...
    # 非法接触终止：只要膝盖或躯干重重砸在地上，立刻终止
    illegal_contact = DoneTerm(
        func=mdp.illegal_contact,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["torso_link", ".*_knee_link"]),
            "threshold": 1.0,
        },
    )
