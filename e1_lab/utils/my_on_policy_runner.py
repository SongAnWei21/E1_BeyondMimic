...
class MyOnPolicyRunner(OnPolicyRunner):
    def save(self, path: str, infos=None):
        ...
        if self.logger_type in ["wandb"]:
            ...
            # Get obs_normalizer from alg if it exists, otherwise use None
            obs_normalizer = getattr(self.alg, 'obs_normalizer', None) if hasattr(self.alg, 'obs_normalizer') else None
...
class MotionOnPolicyRunner(OnPolicyRunner):
    ...
    def save(self, path: str, infos=None):
        ...
        if self.logger_type in ["wandb"]:
            ...
            if self.registry_name is not None and self.registry_name != "local":
