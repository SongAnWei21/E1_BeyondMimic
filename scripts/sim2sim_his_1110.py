"""Unified sim2sim script for E1 robot (10-frame Term-wise History, 1110 obs)."""
...
def run_simulation(motion_file: str, xml_path: str, policy_path: str):
    config = ROBOT_CONFIG
    print(f"[INFO]: E1 (1110 obs, 完美初态降落防炸机版 + 动作平滑滤波)")
    ...
    # 4. 让机器人用第一帧的舞姿，平稳落到地上 (运行 100 步，即 0.5 秒)
    for _ in range(100):
    ...
    # 低通滤波范围 0.0 ~ 1.0，越小动作越丝滑，但会有微小的延迟；越大动作越干脆。
    alpha = 1.0
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                # 【防炸机改进 6：契合大模型的 Term-wise 拼接】
                obs = np.concatenate([
                    hist_cmd.reshape(-1),         # 420
                    hist_proj_grav.reshape(-1),   # 30
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy_path", type=str, default="policy/his_1110/model_50k.onnx")
